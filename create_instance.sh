...
# instance creation
aws ec2 run-instances --image-id $IMAGE_ID --count 1 --instance-type $INSTANCE_TYPE --key-name $KEY_NAME --security-group-ids $SEC_GROUP_ID > $OUT_FILE
sleep 60
...
echo -e "\tYou can use Instance IP and attached .pem(command to set proper permision for .pem ke: chmod 400 hadoop_admin.pem) key to login the servers\nUsername: ec2-user\nLogin syntax: ssh -i "hadoop_admin.pem" ec2-user@YOUR_PUBLIC_IP" > .public_ip.txt
